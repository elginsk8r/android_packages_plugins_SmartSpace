...
public class BcSmartspaceView extends FrameLayout
        ...
        mOnPageChangeListener =
                new ViewPager.OnPageChangeListener() { // from class:
                    ...
                    public void onPageScrollStateChanged(int state) {
                        ...
                        mScrollState = state;
                        if (state == 0 && (list = mPendingTargets) != null) {
                    ...
                    public void onPageScrolled(
                        ...
                        if (mPageIndicator != null) {
                            mPageIndicator.setPageOffset(position, positionOffset);
                        }
                    }

                    public void onPageSelected(int position) {
                        SmartspaceTarget targetAtPosition =
                                mAdapter.getTargetAtPosition(mCardPosition);
                        mCardPosition = position;
                        SmartspaceTarget targetAtPosition2 = mAdapter.getTargetAtPosition(position);
                        logSmartspaceEvent(
                                targetAtPosition2,
                                mCardPosition,
    ...
    public void onVisibilityAggregated(boolean isVisible) {
        ...
        if (mDataProvider != null) {
            mDataProvider.notifySmartspaceEvent(
    ...
    public void onSmartspaceTargetsUpdated(List<? extends Parcelable> list) {
        ...
        mLastReceivedTargets.addAll(
                (Collection)
                        mAdapter.mSmartspaceTargets.stream()
                                .map(
                                        (v0) -> {
    ...
    public final int getCurrentCardTopPadding() {
        ...
        CardPagerAdapter.ViewHolder viewHolder =
                mAdapter.mViewHolders.get(mViewPager.getCurrentItem());
    ...
    public void setDozeAmount(float f) {
        ...
        if (mAdapter.mNextAlarmData.mImage != null) {
            ...
            if (!TextUtils.isEmpty(mAdapter.mNextAlarmData.getHolidayAlarmText(null))) {
                logSmartspaceEvent(
                        mAdapter.mNextAlarmData.mHolidayAlarmsTarget, 0, bcSmartspaceEvent);
            }
        }
    }
